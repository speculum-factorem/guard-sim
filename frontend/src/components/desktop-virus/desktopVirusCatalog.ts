...
export const DESKTOP_VIRUS_CATALOG: readonly DesktopVirusCatalogEntry[] = [
  {
    ...
    goal: "Откройте Антивирус с дока или из «Показать приложения» и запустите режим «Антималварь». Не допустите слишком большой стопки окон.",
  },
  {
    id: "file_worm",
    name: "Файловый червь",
  ...
  },
  {
    ...
    goal:
      "Диспетчер задач (док или меню приложений), вкладка «Сеть» — оцените нагрузку. Брандмауэр — заблокируйте все вредоносные IP, не трогая доверенные. Уложитесь в таймер.",
  },
] as const;
